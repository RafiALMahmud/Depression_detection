...
from app.schemas.vision import LiveEmotionResponse, VisionModelStatusResponse, VisionPredictionResponse
...
@router.post("/predict-frame", response_model=LiveEmotionResponse, status_code=status.HTTP_200_OK)
async def predict_single_frame(
    frame: UploadFile = File(..., description="A single webcam frame for live emotion overlay."),
    ...
    try:
        result = service.predict_single_frame(payload, top_k=top_k)
    ...
    return LiveEmotionResponse.model_validate(result)
